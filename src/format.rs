...
pub fn output(entry: Vec<PathBuf>){
    ...
    let row_end_string = "-".repeat(49+file_size_len+file_name_len);

    println!("{}", row_end_string);
    println!("{:<11} | {:5} | {:<width$} | {:<19} | {:width_file_name$} |", "Permissions", "Owner", "Size", "Last modified", "Name", width=file_size_len, width_file_name=file_name_len);
    println!("{}", row_end_string);
    ...
    for entry in entry{
        ...
        if entry_string.starts_with('.') && !entry.is_dir(){
            print_output(&file_name_output.cyan(), &file_size_output, &file_permissions, &mtime, &file_owner, &file_group)
        }else if entry.is_dir(){
            print_output(&file_name_output.bold().blue(), &file_size_output, &file_permissions, &mtime, &file_owner, &file_group)
        }else if entry.is_symlink(){
            print_output(&file_name_output.yellow(), &file_size_output, &file_permissions, &mtime, &file_owner, &file_group)
        }else if entry.is_executable(){
            print_output(&file_name_output.bold().green(), &file_size_output, &file_permissions, &mtime, &file_owner, &file_group)
        }else{
            print_output(&file_name_output.white(), &file_size_output, &file_permissions, &mtime, &file_owner, &file_group)
        }
    }
    println!("{}", row_end_string);
}
